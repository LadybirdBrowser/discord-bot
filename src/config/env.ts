...
const envSchema = z.object({
  ...
  DISCORD_TOKEN: z
    .string("DISCORD_TOKEN was not found, either it or the .env file are missing")
    .trim()
    .min(1, "DISCORD_TOKEN was not of the minimum length it is either malformed or incomplete"),
  GITHUB_TOKEN: z
    .string("GITHUB_TOKEN was not found, either it or the .env file are missing")
    .trim()
    .min(1, "GITHUB_TOKEN was not of the minimum length it is either malformed or incomplete")
    .optional(),
  GUILD_ID: z
    .string("GUILD_ID was not found, either it or the .env file are missing")
    .trim()
...
declare global {
  ...
  namespace NodeJS {
    // eslint-disable-next-line @typescript-eslint/no-empty-object-type -- Just an overwrite of types

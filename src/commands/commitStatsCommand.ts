...
import githubAPI, { type Commit, type Repository } from "@/apis/githubAPI";
...
export class CommitStatsCommand extends Command {
  ...
  override async handleCommand(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    try {
      ...
      const result = await calculateCommitStats(author);
      ...
      const { user, title, description, color, name, total, userCommits } = result.value;
        ...
        .addFields(
          ...userCommits
            ...
            .map(({ repo, commits, totalCount }, index) => ({
              name: `${toMedal(index + 1)} **${repo.owner}/${repo.name}** - **${(
                totalCount ?? 0
              ).toLocaleString("en-US")} commit${(totalCount ?? 0) > 1 ? "s" : ""}**`,
              value: [
                ...commits
                  .slice(0, 3)
                  .map(
                    ({ commit, sha, html_url: url }) =>
                      `\u200b\u2001- ${trimString(commit.message.split("\n")[0], 45)} ([${sha.slice(
                        0,
                        7
                      )}](${url})).`
                  ),
...
const milestonesCopy: Array<{
    ...
    copy: {
      ...
      description: () =>
        [
          ...
          "[take a look at these issues](<https://github.com/LadybirdBrowser/ladybird/issues?q=is%3Aopen+is%3Aissue+label%3A%22good+first+issue%22>).",
        ].join(" "),
    },
  },
  {
  ...
  },
  {
    ...
    copy: {
      ...
      title: ({ name, total }) =>
        `${name} has so far landed a total of ${total} across the Ladybird project(s)`,
    },
  },
  {
    min: 50,
  ...
  },
  {
    ...
    copy: {
      ...
      title: ({ name, total }) =>
        `${name} is a "1000 Commit Club" member with a whopping ${total} contributed to Ladybird`,
    },
  },
  {
    min: 10000,
...
// TODO: Performance improvements takes minimum 16 seconds for Andreas
export async function calculateCommitStats(author: string): Promise<
  Result<{
    title: (a: { [index: string]: string }) => string;
    description: (a: { [index: string]: string }) => string;
    color: (a: { [index: string]: string }) => string;
    name: string;
    total: string;
    // eslint-disable-next-line -- TODO type properly
    user: any;
    userCommits: RepoInfo[];
  }>
> {
  const user = await githubAPI.getUser(author);
  if (!user) {
    return { ok: false, error: `We looked everywhere; but we couldn't find \`${author}\` :^(` };
  }
  ...
  const totalCommits = userCommits.reduce((n, { totalCount }) => (totalCount ?? 0) + n, 0);
  ...
  return { ok: true, value: { user, title, description, color, name, total, userCommits } };
}
